...
const uint16_t kMinUnknownSize = 150;      //NNR Was: 10
...
const uint8_t kTolerancePercentage = 50; //NNR    was: kTolerance;  // kTolerance is normally 25%
...
void loop() {
    ...
    if (irrecv.decode(&results)) {
        ...
        uint32_t now = millis();
//NNR        Serial.printf(D_STR_TIMESTAMP " : %06u.%03u\n", now / 1000, now % 1000);
        // Check if we got an IR message that was to big for our capture buffer.
        if (results.overflow)
            Serial.printf(D_WARN_BUFFERFULL "\n", kCaptureBufferSize);
        // Display the library version the message was captured with.
//NNR        Serial.println(D_STR_LIBRARY "   : v" _IRREMOTEESP8266_VERSION_ "\n");
        // Display the tolerance percentage if it has been change from the default.
        if (kTolerancePercentage != kTolerance)
//NNR            Serial.printf(D_STR_TOLERANCE " : %d%%\n", kTolerancePercentage);
...
//NNR        Serial.println(resultToSourceCode(&results));
//NNR        Serial.println();    // Blank line between entries
        yield();             // Feed the WDT (again)
    }
    OTAloopHandler();
}
